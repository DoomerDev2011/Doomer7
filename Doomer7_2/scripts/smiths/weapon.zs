Class CK7_Smith_Weapon : Weapon abstract
{
	...
	*/
	...
	Action Void K7_FireBullet(int damage, double spread, double crit = 8, bool bleed = false)
	{
		...
		Puff.Angle = atan2(hitscan.results.hitvector.y, hitscan.results.hitvector.x); //VectorAngle( Dir.x, Dir.y );
		Puff.Pitch = -asin(hitscan.results.hitvector.z); //-VectorAngle( ( Dir.xy.length(),Dir.z) );
		puff.target = self;
		If(Hitscan.victim)
		{
			...
			{
				If(!hitscan.victim.bDONTTHRUST) hitscan.victim.vel += hitscan.results.hitvector*Ouch*10/hitscan.victim.mass;
